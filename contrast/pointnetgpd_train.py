...
if args.cuda:
    torch.cuda.manual_seed(args.gpu)
if args.gpu_num > 1:
    torch.cuda.manual_seed_all(args.gpu)
...
model_machine = model_utils.ModelInit(args.mode, args.load_path, model_params, grasp_points_num, point_channel, class_num)
...
def main():            
    if args.mode == 'train':
        refineModule = PointNetGPDModule(resume_epoch, args.epoch, train_loader, val_loader, test_loader, cur_width=0.08)
        refineModule.train()

    elif args.mode == 'validate':
        refineModule = PointNetGPDModule(resume_epoch, val_data_loader=val_loader, cur_width=0.08)
        refineModule.validate(resume_epoch)

    elif args.mode == 'test':
        refineModule = PointNetGPDModule(resume_epoch, test_data_loader=test_loader, cur_width=0.08)
