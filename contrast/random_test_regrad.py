...
class RandomModule():
    def __init__(self, train_data_loader=None, 
        ...
        self.eval_machine   = model_utils.Eval(self.log_machine, center_num, topK, dis_thre, 0, regrad=True)
    ...
    def train_val(self, mode='train', use_eval=True):
        ...
        for batch_idx, (pc, grasps, labels, data_path) in enumerate(dataloader):
            ...
            if use_eval:
                self.eval_machine.eval_batch(data_path, None, \
                        self.eval_params, self.gripper_params, batch_idx, mode)
        if use_eval:
