...
model_machine = model_utils.GPDModelInit('test', args.load_path, model_params, grasp_points_num, args.project_chann)
...
def main():            
    ...
    if args.eval_diffwidth:
        ...
        for eval_width in eval_widths:
            ...
            model = GPDModule(resume_epoch, test_data_loader=test_loader, cur_width=eval_width)
            model.test(resume_epoch)

    else:
        test_dataset  = PointNetGPDDataset(
                        all_points_num = all_points_num,
                        path = os.path.join(args.data_path, args.eval_width),
                        tag = 'test',
                        data_seed = 1,
                        thresh_score = thresh_score,
                        frame_num = 16
        ...
        test_loader  = utils.get_dataloader(test_dataset, args.batch_size, shuffle=True)
        model = GPDModule(resume_epoch, test_data_loader=test_loader, cur_width=args.eval_width)
        model.test(resume_epoch)
