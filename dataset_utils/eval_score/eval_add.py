...
def eval_test_batch(points, predicted_grasp, table_height, depth, width, gpu: int):
    ...
    frame_r      = frame_r.view(B, K, 3, 3).view(-1,3,3)
    frame_center = frame_center.view(B, K, 3).view(-1,3)
    frame_score  = frame_score.view(B, K, 1)
    # print(frame_center[:,2] + frame_r[:,2, 0] * depth)
    # collision_table_bool = (frame_center[:,2] + frame_r[:,2, 0] * depth < table_height - 0.005)
    # print(collision_table_bool.sum())
    # frame_center[collision_table_bool] += frame_r[collision_table_bool,2,0] * 0.1
    ...
    return keep_idx, keep_num, frame_center
