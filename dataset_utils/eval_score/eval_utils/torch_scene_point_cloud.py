...
class TorchScenePointCloud(PointCloud):
    def __init__(self, data, voxelize=True, visualization=False, radius=-1):
        points = data['scene_cloud']
        # points = np.r_[data['scene_cloud'], data['view_cloud']]#data['scene_cloud']
        ...
        if 'scene_normal' in data.keys():
            ...
            PointCloud.__init__(self, cloud, visualization=visualization)
            self.estimate_normals(radius=radius)
